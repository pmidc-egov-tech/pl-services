...
import static org.egov.pl.util.BPAConstants.*;
import static org.egov.pl.util.PLConstants.NOTIFICATION_LOCALE;
...
public class BPANotificationUtil {
    ...
    @Autowired
    public BPANotificationUtil(PLConfiguration config, ServiceRequestRepository serviceRequestRepository,
                               Producer producer) {
        this.config = config;
...
//                break;
     ...
     */
    private String getPendingPaymentMsg(PetLicense license, String message, String localizationMessage) {
        message = message.replace("<2>", license.getApplicationNumber());
        String categoryName = license.getPetLicenseDetail().getAnimalCategory().getName();
...
//        message = message.replace("<5>", egovhost + payEndpoint);
    ...
    public String getPendingDocVerificationMsg(PetLicense license, String message, String localizationMessage, String totalAmountPaid) {
     ...
     */
    private String getApprovedMsg(PetLicense license, String message, String localizationMessage) {
        message = message.replace("<2>", license.getApplicationNumber());
        String categoryName = license.getPetLicenseDetail().getAnimalCategory().getName();
//        message = message.replace("<3>", getMessageTemplate(TRADE_LOCALISATION_PREFIX + licensetype.split("\\.")[0], localizationMessage));
//        message = message.replace("<4>", getMessageTemplate(TRADE_LOCALISATION_PREFIX + licensetype.replace(".", "_"), localizationMessage));
     ...
     */
    private String getPendingApprovalMsg(PetLicense license, String message) {
        message = message.replace("<2>", license.getApplicationNumber());
        return message;
    }
}
