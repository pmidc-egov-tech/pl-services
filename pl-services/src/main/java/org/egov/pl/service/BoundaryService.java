...
public class BoundaryService {
    ...
    @Autowired
    public BoundaryService(ServiceRequestRepository serviceRequestRepository, ObjectMapper mapper, PLConfiguration config) {
     ...
     */
    public void getAreaType(PetLicenseRequest request, String hierarchyTypeCode){
        ...
        request.getLicenses().forEach(license -> {
            if(license.getPetLicenseDetail().getAddress()==null ||
                    license.getPetLicenseDetail().getAddress().getLocality()==null)
                throw new CustomException("INVALID ADDRESS","The address or locality cannot be null");
            localities.add(license.getPetLicenseDetail().getAddress().getLocality().getCode());
        });
        ...
        request.getLicenses().forEach(license -> {
            ...
            if(boundary.getName()==null)
                throw new CustomException("INVALID BOUNDARY DATA","The boundary data for the code "+license.getPetLicenseDetail().getAddress().getLocality().getCode()+ " is not available");
            license.getPetLicenseDetail().getAddress().setLocality(boundary);
     ...
     * @param request PetLicenseRequest for create
     * @return Map of tradeLcienseId to jsonPath with tradeLcienses locality code
     */
    private Map<String,String> getJsonpath(PetLicenseRequest request){
        Map<String,String> idToJsonPath = new LinkedHashMap<>();
        String jsonpath = "$..boundary[?(@.code==\"{}\")]";
        request.getLicenses().forEach(license -> {
            idToJsonPath.put(license.getId(),jsonpath.replace("{}",license.getPetLicenseDetail().getAddress().getLocality().getCode()
            ));
        });
