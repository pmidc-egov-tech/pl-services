...
public class PLNotificationService {
	...
	@Autowired
	public PLNotificationService(PLConfiguration config, ServiceRequestRepository serviceRequestRepository,
	 ...
	 * 
	 * @param request     The petLicenseRequest from kafka topic
	...
	private void enrichSMSRequest(PetLicenseRequest request, List<SMSRequest> smsRequests) {
		...
		for (PetLicense license : request.getLicenses()) {
			...
			if (businessService.equals(businessService_PL)) {
				...
				String localizationMessages = bpaNotificationUtil.getLocalizationMessages(tenantId,
	 ...
	 * 
	 * Assumption - The TradeLicenseRequest received will always contain only one
	 * PetLicense.
	 * 
	 * @param request
	 * @return
	 */
	private EventRequest getEventsForPL(PetLicenseRequest request) {
